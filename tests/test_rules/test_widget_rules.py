...
from phoebuslint.rules.widget import WidgetOutOfBounds, WidgetHeightOrWidthZeroOrNegative
...
def test_widget_height_or_width_zero_or_negative_rule(sample_empty_screen: Screen):
    ...
    assert (
        WidgetHeightOrWidthZeroOrNegative.check(sample_empty_screen) is None
    )

    label_zero_width = Label("label2", "Zero Width", 20, 50, 0, 30)
    sample_empty_screen.add_widget(label_zero_width)
    assert (
        WidgetHeightOrWidthZeroOrNegative.check(sample_empty_screen) is not None
    )
