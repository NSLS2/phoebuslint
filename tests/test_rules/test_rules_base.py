...
def test_rule_violation_factory():
    ...
    all_rule_classes = LintRule.__subclasses__() + RecursiveLintRule.__subclasses__()
    for rule_cls in all_rule_classes:
        violation = rule_cls.rule_violation_factory(screen=Screen(f_name="test_screen.bob"))  # screen can be None for this test
        assert violation.rule_code == rule_cls.rule_code, f"Rule code mismatch in {rule_cls.__name__}"
        assert violation.rule_severity == rule_cls.rule_severity, f"Rule severity mismatch in {rule_cls.__name__}"
        assert violation.screen.bob_file == "test_screen.bob", f"Screen file mismatch in {rule_cls.__name__}"
