...
def test_navigation_step_equality(nav_step_factory):
    ...
    assert step1 == step2, (
        "Steps with same from_screen, to_screen, and macros should be equal"
    )
...
def test_screen_nav_dag_from_empty_screen(sample_empty_screen):
    ...
    assert isinstance(dag, ScreenNavigationDAG), (
        "Should return a ScreenNavigationDAG instance"
    )
...
def test_screen_nav_dag_with_linked_screens():
    ...
    assert dag.graph.get(Path("examples/Example_TOP1.bob")) is not None, (
        "Graph should contain the root screen"
    )
    assert dag.graph.get(Path("examples/Example_TOP1.bob")) == [
        NavigationStep(
            from_screen=Path("examples/Example_TOP1.bob"),
            to_screen=Path("examples/Nested/Nested1.bob"),
            macros={"A": "1", "B": "2"},
        )
    ], "Graph should contain the correct navigation step from TOP1 to Nested1"
